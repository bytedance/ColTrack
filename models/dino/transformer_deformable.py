...
class DeformableTransformer(nn.Module):
    ...
    def get_proposal_pos_embed(self, proposals):
        ...
        # N, L, 4
        proposals = scale_sigmoid(proposals.sigmoid()) * scale
        # N, L, 4, 128
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None):
        ...
        if self.two_stage:
            ...
            reference_points = scale_sigmoid(topk_coords_unact.sigmoid())
            init_reference_out = reference_points
            pos_trans_out = self.pos_trans_norm(self.pos_trans(self.get_proposal_pos_embed(topk_coords_unact)))
            query_embed, tgt = torch.split(pos_trans_out, c, dim=2)
        elif self.use_dab:
            reference_points = scale_sigmoid(query_embed[..., self.d_model:].sigmoid()) 
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes,       
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if self.bbox_embed is not None:
                ...
                new_reference_points = scale_sigmoid(box_holder[..., :self.query_dim].sigmoid())
