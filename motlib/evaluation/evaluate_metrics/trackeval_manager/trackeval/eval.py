        ...
        for dataset, dataset_name in zip(dataset_list, dataset_names):
            ...
            for tracker in tracker_list:
                ...
                try:
                    ...
                    self.logger.info('Tracker %s was unable to be evaluated.' % tracker)
                    self.logger.error(err)
                    traceback.print_exc()
                    if config['LOG_ON_ERROR'] is not None:
                        with open(config['LOG_ON_ERROR'], 'a') as f:
                            self.logger.info(dataset_name, file=f)
                            self.logger.info(tracker, file=f)
                            self.logger.info(traceback.format_exc(), file=f)
                            self.logger.info('\n\n\n', file=f)
