...
class ETHZ(DatasetFileBase):
    def init(self):
        super().init()
        self.subdirectory = 'ETHZ'
...
def load_paths(data_path):
    with open(data_path, 'r') as file:
        ...
        img_files = [x.replace('\n', '').replace('ETHZ/', '') for x in img_files]
...
def load_dataset_core(data_path=''):
    ...
    img_paths, label_paths  = load_paths(str(cur_dir / 'data_path/eth.train'))
    ...
    for img_path, label_path in zip(img_paths, label_paths):
        ...
        video_info = {'height':img_height,'width': img_width, 'frame_rate':-1, 'dataset': 'ETHZ', 'name': img_path.split('/')[0]}
        frame_info = {}
